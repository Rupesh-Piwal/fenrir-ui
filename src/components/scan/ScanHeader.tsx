import { scanMeta } from "../../data/scanData";
import { Search, Network, FlaskConical, ClipboardCheck, FileText } from "lucide-react";
            ...
            <div className="flex-1 md:pl-8 flex flex-col justify-between">
                        ...
                        {scanMeta.stages.map((stage, index) => {
                            ...
                            return (
                                <div key={stage} className="relative z-10 flex flex-col items-center group">
                                    <div
                                        className={`w-12 h-12 rounded-full flex justify-center items-center mb-3 transition-colors ${isActive ? "bg-[#0b2824] border border-accent/20" : "bg-surface border border-border text-text-secondary"
                                            }`}
                                    >
                                        {getStageIcon(index)}
                                    </div>
                ...
                <div className="grid grid-cols-2 lg:grid-cols-6 gap-4">
                    <div>
                        ...
                        <p className="text-sm font-semibold text-text-primary">{scanMeta.scanType}</p>
                    </div>
                    <div>
                        <p className="text-xs text-text-secondary mb-1">Targets</p>
                        <p className="text-sm font-semibold text-text-primary">{scanMeta.target}</p>
                    </div>
                    <div>
                        <p className="text-xs text-text-secondary mb-1">Started At</p>
                        <p className="text-sm font-semibold text-text-primary">{scanMeta.startedAt}</p>
                    </div>
                    <div>
                        <p className="text-xs text-text-secondary mb-1">Credentials</p>
                        <p className="text-sm font-semibold text-text-primary">{scanMeta.credentials}</p>
                    </div>
                    <div>
                        <p className="text-xs text-text-secondary mb-1">Files</p>
                        <p className="text-sm font-semibold text-text-primary">{scanMeta.files}</p>
                    </div>
                    <div>
                        <p className="text-xs text-text-secondary mb-1">Checklists</p>
                        <p className="text-sm font-semibold text-accent">{scanMeta.checklists}</p>
                    </div>
                </div>
            </div>
        </div>
